...
worldpop = rio.open(RAW_DATA_DIRECTORY / 'worldpop/2015.tif').read(1)
downscale_factor = 1
...
# Plot
im = ax.imshow(worldpop, cmap=mycmp)
...
# Define inset area.
x_min = int(worldpop.shape[0]*0.59)
...
ax.axis('off')
plt.savefig(DATAVIZ_DIRECTORY / 'worldpop_zoom.pdf', bbox_inches=mpl.transforms.Bbox(np.array([[-0.1, 0], [7.2, 7.2]])))
#plt.show()
